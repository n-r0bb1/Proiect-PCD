/* motion_detection.c – OpenCV frame detectare mișcare în frame-uri (C API) */
...
long motion_detect_chunk(const char *video_path,
                         long        start_frame,
                         long        end_frame,
                         int         threshold) //Detecteaza numarul de frame-uri fara miscare din chunk-ul dat
{
    ...
    IplImage  *curr_frame      = NULL; //Frame-ul curent citit din video (owned by capture)
    ...
    if (video_path == NULL || start_frame < 0 || end_frame <= start_frame || threshold < 0) { //Validare parametri de intrare
    ...
    if (!cvSetCaptureProperty(cap, CV_CAP_PROP_POS_FRAMES, (double)start_frame)) { //Mutare la frame-ul de start
    ...
    for (idx = start_frame; idx < end_frame; ++idx) { //Parcurgere frame-uri din chunk
        ...
        if (max_val <= (double)threshold) { //Diferenta sub prag – considerat fara miscare
            ++no_motion_count;
        }
    ...
    if (prev_gray != NULL) { cvReleaseImage(&prev_gray); } //Eliberare buffer frame anterior
    if (curr_gray != NULL) { cvReleaseImage(&curr_gray); } //Eliberare buffer frame curent
    if (diff      != NULL) { cvReleaseImage(&diff);      } //Eliberare buffer diferenta
    cvReleaseCapture(&cap);                                 //Eliberare handle captura video

    return no_motion_count; //Returneaza numarul total de frame-uri fara miscare din chunk
}